...
export class ReviewService {
    ...
    return this.prisma.$transaction(async (tx) => {
      const review = await tx.review.create({
        data: {
          ...
          versions: { create: { rating, content } },
        },
  ...
  async update(user: AuthenticatedUser, reviewId: string, dto: UpdateReviewDto) {
    ...
    return this.prisma.$transaction(async (tx) => {
      const updated = await tx.review.update({
        where: { id: reviewId },
        data: { rating, content, isSpoiler, versions: { create: { rating, content } } },
        include: { user: { select: { id: true, username: true, profile: { select: { avatarUrl: true } } } } },
      });
  ...
  async remove(user: AuthenticatedUser, reviewId: string): Promise<{ success: true }> {
    ...
    if (!review) throw new NotFoundException('Avaliação não encontrada.');
    if (review.userId !== user.id && user.role !== 'ADMIN' && user.role !== 'MODERATOR') {
  ...
  async toggleLike(user: AuthenticatedUser, reviewId: string): Promise<{ liked: boolean; likeCount: number }> {
    const review = await this.prisma.review.findUnique({ where: { id: reviewId }, select: { id: true, likeCount: true } });
    if (!review) throw new NotFoundException('Avaliação não encontrada.');
  ...
  async addComment(user: AuthenticatedUser, reviewId: string, dto: CreateReviewCommentDto) {
    ...
    return this.prisma.$transaction(async (tx) => {
      const comment = await tx.reviewComment.create({
        data: { userId: user.id, reviewId, content, isSpoiler: Boolean(dto.isSpoiler) },
        include: { user: { select: { id: true, username: true, profile: { select: { avatarUrl: true } } } } },
      });
  ...
  private normalizeRating(value: unknown): number {
    ...
    if (!Number.isFinite(rating) || rating < 1 || rating > 5) throw new BadRequestException('A nota deve estar entre 1 e 5.');
    return Math.round(rating * 2) / 2;
  }

  private normalizeContent(value: unknown, maxLength: number): string {
    const content = String(value ?? '').trim();
    if (!content) throw new BadRequestException('O conteúdo é obrigatório.');
    if (content.length > maxLength) throw new BadRequestException(`O conteúdo deve ter no máximo ${maxLength} caracteres.`);
    return content;
  }

  private async recalculateMediaRating(tx: any, mediaId: string): Promise<void> {
    const aggregate = await tx.review.aggregate({ where: { mediaId }, _avg: { rating: true }, _count: { _all: true } });
