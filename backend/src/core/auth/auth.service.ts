...
export class AuthService {
  ...
  async register(dto: RegisterDto, metadata: { ip?: string; userAgent?: string }): Promise<IssuedSession> {
    ...
    if (!/^[a-zA-Z0-9_.-]{3,32}$/.test(username)) {
      throw new BadRequestException('Username deve ter entre 3 e 32 caracteres e conter apenas letras, números, _, - ou .');
    }
    ...
    const user = await this.prisma.user.create({
      data: { email, username, passwordHash, profile: { create: {} } },
  ...
  async login(dto: LoginDto, metadata: { ip?: string; userAgent?: string }): Promise<IssuedSession> {
    ...
    const user = await this.prisma.user.findUnique({
      where: { email },
      select: { id: true, email: true, username: true, role: true, isBanned: true, passwordHash: true },
    });
    if (!user || user.isBanned || !(await this.verifyPassword(password, user.passwordHash))) {
  ...
  async refresh(refreshToken: string, metadata: { ip?: string; userAgent?: string }): Promise<IssuedSession> {
    ...
    const session = await this.prisma.session.findUnique({
      ...
      include: { user: { select: { id: true, email: true, username: true, role: true, isBanned: true } } },
    });
    if (!session || session.tokenHash !== this.hashToken(refreshToken) || session.expiresAt <= new Date() || session.user.isBanned || session.user.id !== payload.sub) {
  ...
  async logout(refreshToken: string): Promise<void> {
    ...
    await this.prisma.session.deleteMany({ where: { tokenHash: this.hashToken(refreshToken) } });
  }
  ...
  getRefreshCookieOptions(maxAge: number) {
    ...
    return { httpOnly: true, secure: production, sameSite: 'lax' as const, path: '/api/v1/auth', maxAge };
  }
  ...
  private async createIssuedSession(user: { id: string; email: string; username: string; role: AuthenticatedUser['role'] }, metadata: { ip?: string; userAgent?: string }): Promise<IssuedSession> {
    ...
    if (!secret || !jwtSecret) throw new UnauthorizedException('Autenticação indisponível no servidor.');
    ...
    const session = await this.prisma.session.create({
      data: {
        ...
        deviceInfo: metadata.userAgent?.slice(0, 500),
        ipAddress: metadata.ip?.slice(0, 100),
        expiresAt: new Date(Date.now() + refreshMaxAgeMs),
      },
    });

    const signed = jwt.sign(
      { sub: user.id, sessionId: session.id, type: 'refresh' } satisfies RefreshTokenPayload,
      secret,
      { expiresIn: (process.env.REFRESH_TOKEN_EXPIRES_IN as JwtExpires) || '7d' },
    );
  ...
  private buildAuthResponse(user: { id: string; email: string; username: string; role: AuthenticatedUser['role'] }): AuthResponse {
    ...
    if (!secret) throw new UnauthorizedException('Autenticação indisponível no servidor.');
    const payload: AccessTokenPayload = { sub: user.id, email: user.email, username: user.username, role: user.role, type: 'access' };
    return {
      accessToken: jwt.sign(payload, secret, { expiresIn: (process.env.JWT_EXPIRES_IN as JwtExpires) || '15m' }),
      user: this.buildUser(user),
    };
  }

  private buildUser(user: { id: string; email: string; username: string; role: AuthenticatedUser['role'] }): AuthenticatedUser {
  ...
  private async verifyPassword(password: string, stored: string): Promise<boolean> {
    ...
    const derived = (await scrypt(password, salt, 64)) as Buffer;
    return derived.toString('hex') === digest;
  }
